...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Since the models provided by orm do not support the meta-field capabilities of Django's ORM have
        # to do more work to get to the Housing Location entry via the Housing History table
        for inmate in orm.CountyInmate.objects.all():
            if inmate.discharge_date_earliest is None:
                housing_history = orm.HousingHistory.objects.filter(inmate_id=inmate.jail_id)
                housing_location_id = housing_history[len(housing_history) - 1].housing_location_id
                inmate.in_jail = orm.HousingLocation.objects.get(housing_location=housing_location_id).in_jail
            else:
