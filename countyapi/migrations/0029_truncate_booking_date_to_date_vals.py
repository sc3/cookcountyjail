...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        populate new field booked_date with the date value from date time field booking_date
        handle case of null booking date by using jail_id to set booked_date
        @param orm:
        @return:
        """
        ...
        for inmate in orm.CountyInmate.objects.all():
            if inmate.booking_date is None:
                inmate.booked_date = datetime.strptime(inmate.jail_id[0:9], '%Y-%m%d').date()
            else:
                inmate.booked_date = inmate.booking_date.date()
            inmate.save()
    ...
    models = {
        ...
        u'countyapi.countyinmate': {
            ...
            'booked_date': ('django.db.models.fields.DateField', [], {'null': 'True'}),
            ...
            'last_seen_date': ('django.db.models.fields.DateTimeField', [], {}),
