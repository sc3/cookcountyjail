...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for inmate in orm.CountyInmate.objects.all():
            new_inmate_charges = inmate.charges_history.create(charges=inmate.charges, charges_citation=inmate.charges_citation)
            new_inmate_charges.date_seen = datetime.datetime.now().date()
            new_inmate_charges.save()


    def backwards(self, orm):
        "Write your backwards methods here."
        for inmate in orm.CountyInmate.objects.all():
            latest_charge = inmate.charges_history.latest('date_seen')
            inmate.charges = latest_charge.charges
            inmate.charges_citation = latest_charge.charges_citation
            inmate.save()
