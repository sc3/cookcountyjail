...
def discharge_inmates(last_date_scraped):
    ...
    Warning:
        ...
        Then the database filter last_seen_date__lt=last_date_scraped would not filter anyone.
        Even though both the inmates and the scrape where in the same day.
    ...
    inmates_to_discharge = CountyInmate.objects.filter(discharge_date_earliest__exact=None,
                                                       last_seen_date__lt=last_date_scraped)
...
def obj_to_datetime(obj_to_parse):
    ...
    If obj_to_parse is a string it needs to be in a format of YYYY-MM-DD, it delegates the parsing to
    'convert_to_datetime'.
    ...
    if isinstance(obj_to_parse, str):
        ...
        if obj_to_parse is None:
            # convert_to_datetime returns None if the string doesn't have three '-' in the string,
            # or if any of the splitted elements isn't convertible to an int object
